...
def convert_sc_all(
    ...
    if dataset_attr.kto_tag and isinstance(example[dataset_attr.kto_tag], bool):  # kto example
        ...
        chosen = example[dataset_attr.chosen]
        rejected = example[dataset_attr.rejected]
        intermediate = example[dataset_attr.intermediate]
        if (
            chosen[dataset_attr.role_tag] not in accept_tags[-1]
            or rejected[dataset_attr.role_tag] not in accept_tags[-1]
            or intermediate[dataset_attr.role_tag] not in accept_tags[-1]
        ):
            logger.warning("Invalid role tag in {}.".format([chosen, rejected]))
            broken_data = True
        ...
        response = [
            {"role": tag_mapping[chosen[dataset_attr.role_tag]], "content": chosen[dataset_attr.content_tag]},
            {"role": tag_mapping[rejected[dataset_attr.role_tag]], "content": rejected[dataset_attr.content_tag]},
            {"role": tag_mapping[intermediate[dataset_attr.role_tag]], "content": intermediate[dataset_attr.content_tag]}
        ]
    else:  # normal example
        prompt = aligned_messages[:-1]
        response = aligned_messages[-1:]
...
def align_dataset(
    ...
    elif dataset_attr.formatting == "sc_all":
        convert_func = partial(convert_sc_all, dataset_attr=dataset_attr, data_args=data_args)
